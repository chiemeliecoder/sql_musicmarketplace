...
LEFT JOIN `musicmarketplace`.`artist` ON `musicmarketplace`.`artist_genre`.`artistid` = `musicmarketplace`.`artist`.`id`
LEFT JOIN `musicmarketplace`.`album` ON `musicmarketplace`.`artist`.`id` = `musicmarketplace`.`album`.`id`
