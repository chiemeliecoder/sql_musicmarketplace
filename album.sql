...
INSERT INTO `album` (`idalbum`, `title`, `date`, `artistid`) VALUES (1, '1989', '2014-10-27', 1);
...
JOIN `musicmarketplace`.`album` ON  `musicmarketplace`.`artist`.`idartist` = `musicmarketplace`.`album`.`idalbum`
GROUP BY `musicmarketplace`.`artist`.`idartist`;

-- Find artists who have released 1 album
SELECT `musicmarketplace`.`artist`.`idartist`, `musicmarketplace`.`artist`.`name`, COUNT(`musicmarketplace`.`album`.`idalbum`) AS num_albums
FROM `musicmarketplace`.`artist`
JOIN `musicmarketplace`.`album` ON `musicmarketplace`.`artist`.`idartist` = `musicmarketplace`.`album`.`idalbum`
GROUP BY `musicmarketplace`.`artist`.`idartist`
HAVING COUNT(`musicmarketplace`.`album`.`idalbum`) >= 1;